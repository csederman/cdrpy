...
class CombinationResponseSequence(Sequence):
    """Keras-compatible data generator to use with the Keras
    methods :meth:`keras.Model.fit`, :meth:`keras.Model.evaluate`,
    and :meth:`keras.Model.predict`.
    ...
    ----------
        ...
        batch_size: The batch size.
        cell_ids: An iterable of cell ids to encode.
        drug_1_ids: An iterable of drug ids to encode.
        drug_2_ids: An iterable of drug ids to encode.
        targets: An iterable of target labels.
        shuffle: Whether or not to shuffle after each iteration.
        seed: A seed for random number generation.
    """
    ...
    def __init__(
        ...
        # check that drug_1_ids is an iterable
        if not is_real_iterable(drug_1_ids):
            raise TypeError(
                "drug_1_ids must be an iterable or a numpy array if drug ids."
            )

        # check that drug_1_ids is an iterable
        if not is_real_iterable(drug_2_ids):
            raise TypeError(
                "drug_2_ids must be an iterable or a numpy array if drug ids."
            )

        if len(set(map(len, [cell_ids, drug_1_ids, drug_2_ids]))) != 1:
            raise ValueError(
                "The length of cell_ids, drug_1_ids, and drug_2_ids must be the same."
            )
        ...
        self.cell_ids = np.asanyarray(cell_ids)
        self.drug_1_ids = np.asanyarray(drug_1_ids)
        self.drug_2_ids = np.asanyarray(drug_2_ids)
        self.data_size = len(cell_ids)
        self.batch_size = batch_size
        self.shuffle = shuffle
        self._rs, _ = random_state(seed)
    ...
    def __getitem__(self, batch_num: int):
        ...
        batch_indices = self.indices[start_idx:end_idx]
        batch_cell_ids = self.cell_ids[batch_indices]
        batch_drug_1_ids = self.drug_1_ids[batch_indices]
        batch_drug_2_ids = self.drug_2_ids[batch_indices]
        ...
        batch_feats = self._encode_function(
            batch_cell_ids, batch_drug_1_ids, batch_drug_2_ids
        )
        return batch_feats, batch_targets, batch_sample_weights
